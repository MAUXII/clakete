...
export function ListCard({ list, className, compact = false }: ListCardProps) {
  ...
  if (compact) {
    return (
      <div
        className={cn(
          "relative flex w-full flex-col items-center overflow-visible rounded-xl px-1 pb-2 pt-1",
          listCardCompactMinHeightClassName,
          className,
        )}
      >
  ...
  return (
    <div
      className={cn(
        "group relative flex h-full min-w-0 flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-none transition-colors duration-200 hover:shadow-none",
        listCardMinHeightClassName,
        className,
      )}
    >
