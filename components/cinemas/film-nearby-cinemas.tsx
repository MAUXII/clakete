...
export function FilmNearbyCinemasSheet({
  open,
  onOpenChange,
  tmdbId,
  title,
  ...
  title: string
  originalTitle?: string | null
  onAvailableChange?: (available: boolean, cinemaCount: number) => void
}) {
  const { t } = useT()
  ...
  const available = !loading && !error && rows.length > 0

  useEffect(() => {
    onAvailableChange?.(available, rows.length)
  }, [available, rows.length, onAvailableChange])
