import SwiftUI

enum DesignTokens {
  enum CornerRadius {
    static let poster: CGFloat = 16
    static let thumbnail: CGFloat = 8
    static let input: CGFloat = 12
    static let badge: CGFloat = 6
    static let card: CGFloat = 14
  }

  enum Spacing {
    static let screenHorizontal: CGFloat = 24
    static let section: CGFloat = 28
    static let tight: CGFloat = 12
  }

  enum Size {
    static let buttonHeight: CGFloat = 52
    static let posterWidth: CGFloat = 120
    static let posterHeight: CGFloat = 180
  }
}

extension Color {
  init(hex: String) {
    let hex = hex.trimmingCharacters(in: CharacterSet.alphanumerics.inverted)
    var int: UInt64 = 0
    Scanner(string: hex).scanHexInt64(&int)
    let a, r, g, b: UInt64
    switch hex.count {
    case 3:
      (a, r, g, b) = (255, (int >> 8) * 17, (int >> 4 & 0xF) * 17, (int & 0xF) * 17)
    case 6:
      (a, r, g, b) = (255, int >> 16, int >> 8 & 0xFF, int & 0xFF)
    case 8:
      (a, r, g, b) = (int >> 24, int >> 16 & 0xFF, int >> 8 & 0xFF, int & 0xFF)
    default:
      (a, r, g, b) = (255, 1, 1, 0)
    }
    self.init(
      .sRGB,
      red: Double(r) / 255,
      green: Double(g) / 255,
      blue: Double(b) / 255,
      opacity: Double(a) / 255
    )
  }

  /// #09090B — fundo Clakete
  static let appBackground = Color(hex: "09090B")
  static let appElevated = Color(hex: "121214")
  static let appForeground = Color(hex: "FAFAFA")
  static let appMuted = Color(hex: "A1A1AA")
  static let appBorder = Color.white.opacity(0.1)
  static let appAccent = Color(hex: "FF0048")
  static let appAccentSoft = Color(hex: "FF0048").opacity(0.14)
  static let appStar = Color(hex: "FBBF24")
}
